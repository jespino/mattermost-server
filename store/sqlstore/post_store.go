...
func (s *SqlPostStore) Save(post *model.Post) (*model.Post, *model.AppError) {
	...
	if post.Type != model.POST_JOIN_LEAVE && post.Type != model.POST_ADD_REMOVE &&
		...
		if s.DriverName() == model.DATABASE_DRIVER_SQLITE {
			if _, err := s.GetMaster().Exec("UPDATE Channels SET LastPostAt = MAX(:LastPostAt, LastPostAt), TotalMsgCount = TotalMsgCount + 1 WHERE Id = :ChannelId", map[string]interface{}{"LastPostAt": time, "ChannelId": post.ChannelId}); err != nil {

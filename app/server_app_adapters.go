...
func (s *Server) RunOldAppInitalization() error {
	...
	if s.FakeApp().Srv.newStore == nil {
		s.FakeApp().Srv.newStore = func() store.Store {
			return store.NewTimerLayer(localcachelayer.NewLocalCacheLayer(sqlstore.NewSqlStore(s.FakeApp().Config().SqlSettings, s.Metrics), s.Metrics, s.Cluster), s.Metrics)
		}
	}
